...
export default function MailPreviewItem({ mail }: MailPreviewItemProps) {
  ...
  return (
    <li className="border-b border-line/60 py-3 last:border-b-0">
      ...
      <p className="mt-0.5 truncate text-sm text-slate-800">{mail.subject}</p>
      <p className="mt-1 line-clamp-1 text-xs text-slate-500">{mail.preview}</p>
      <p className="mt-1 text-xs font-medium text-slate-400">{mail.timeLabel}</p>
    </li>
  )
}
